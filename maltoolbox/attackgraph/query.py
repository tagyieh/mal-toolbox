...
def is_node_traversable_by_attacker(node: AttackGraphNode, attacker: Attacker) -> bool:
    ...
    """
    logger.debug(
        'Evaluate if "%s"(%d), of type "%s", is traversable by Attacker ' '"%s"(%d)',
        node.full_name,
        node.id,
        node.type,
        attacker.name,
    ...
    match node.type:
        case 'or':
            logger.debug(
                '"%s"(%d) is traversable because it is viable and ' 'of type "or".',
                node.full_name,
                node.id,
            )
            return True
        ...
        case 'and':
            for parent in node.parents:
                if parent.is_necessary and not parent.is_compromised_by(attacker):
...
def get_attack_surface(attacker: Attacker) -> list[AttackGraphNode]:
    ...
    """
    logger.debug(
        'Get the attack surface for Attacker "%s"(%d).', attacker.name, attacker.id
    )
    attack_surface = []
    for attack_step in attacker.reached_attack_steps:
        logger.debug(
            'Determine attack surface stemming from ' '"%s"(%d) for Attacker "%s"(%d).',
        ...
        for child in attack_step.children:
            if (
                is_node_traversable_by_attacker(child, attacker)
...
def update_attack_surface_add_nodes(
    ...
    """
    logger.debug(
        'Update the attack surface for Attacker "%s"(%d).', attacker.name, attacker.id
    )
    ...
    for attack_step in nodes:
        ...
        for child in attack_step.children:
            ...
            if is_traversable and child not in attack_surface:
                logger.debug(
                    'Add node "%s"(%d) to the attack surface of ' 'Attacker "%s"(%d).',
                    child.full_name,
                    child.id,
                    attacker.name,
                    attacker.id,
