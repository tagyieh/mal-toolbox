...
def get_model(
    ...
    # Get all assets
    assets_results = g.run(
        'MATCH (a) WHERE a.type IS NOT NULL RETURN DISTINCT a'
    ).data()
    for asset in assets_results:
        ...
        asset_obj = getattr(lang_classes_factory.ns, asset_data['type'])(
    ...
    for assoc in assocs_results:
        ...
        if attacker_id is not None:
            ...
            attacker.entry_points.append((target_asset, [target_prop]))
            continue
        ...
        assoc_name = lang_classes_factory.get_association_by_signature(
            assoc.name, left_asset.type, right_asset.type
        )
        ...
        if not (
            instance_model.association_exists_between_assets(
                assoc_name, left_asset, right_asset
            )
            or instance_model.association_exists_between_assets(
                assoc_name, right_asset, left_asset
            )
        ):
