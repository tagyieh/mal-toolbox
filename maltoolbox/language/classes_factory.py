...
class LanguageClassesFactory:
    ...
    def _generate_assets(self) -> None:
        ...
        for asset in self.lang_graph.assets:
            ...
            for defense in filter(
                lambda step: step.type == 'defense', asset.attack_steps
            ):
    ...
    def _generate_associations(self) -> None:
        ...
        def create_association_with_subentries(assoc: SchemaGeneratedClass):
            if (
                ...
                self.json_schema['definitions']['LanguageAssociation']['definitions'][
                    assoc.name
                ] = {
                    'title': assoc.name,
                    'type': 'object',
                ...
                self.json_schema['definitions']['LanguageAssociation']['oneOf'].append(
                    {
            ...
            self.json_schema['definitions']['LanguageAssociation']['definitions'][
                assoc.name
            ]['definitions'][subentry_name] = assoc_json_subentry
            self.json_schema['definitions']['LanguageAssociation']['definitions'][
                assoc.name
            ]['oneOf'].append(
                {
        ...
        for assoc in self.lang_graph.associations:
            count = len(
                list(
                    filter(
                        lambda temp_assoc: temp_assoc.name == assoc.name,
                        self.lang_graph.associations,
                    )
                )
            )
            if count > 1:
