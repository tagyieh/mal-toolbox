...
class LanguageGraphAsset:
    name: str | None = None
    associations: list[LanguageGraphAssociation] = field(default_factory=list)
    attack_steps: list[LanguageGraphAttackStep] = field(default_factory=list)
    description: dict = field(default_factory=dict)
    # MAL languages currently do not support multiple inheritance, but this is
    # futureproofing at its most hopeful.
    super_assets: list = field(default_factory=list)
    sub_assets: list = field(default_factory=list)
    ...
    def to_dict(self) -> dict:
        ...
        for assoc in self.associations:
            node_dict['associations'].append(
                (assoc.name, assoc.left_field.fieldname, assoc.right_field.fieldname)
            )
...
class LanguageGraphAssociation:
    ...
    def get_opposite_fieldname(self, fieldname: str) -> str:
        ...
        msg = (
            'Requested fieldname "%s" from association ' '%s which did not contain it!'
        )
    ...
    def get_opposite_asset(
        ...
        logger.warning(
            'Requested asset "%s" from association %s' 'which did not contain it!',
            asset.name,
            self.name,
        )
        return None
    ...
    def to_dict(self) -> dict:
        ...
        for child in self.children:
            ...
            for _, dep_chain in self.children[child]:
                if dep_chain:
                    node_dict['children'][child].append(dep_chain.to_dict())
                else:
        ...
        for parent in self.parents:
            ...
            for _, dep_chain in self.parents[parent]:
                if dep_chain:
                    node_dict['parents'][parent].append(dep_chain.to_dict())
                else:
...
class DependencyChain:
    def __init__(self, type: str, next_link: DependencyChain | None):
        ...
        self.fieldname: str = ''
        self.association: LanguageGraphAssociation | None = None
        self.left_chain: DependencyChain | None = None
        self.right_chain: DependencyChain | None = None
        self.subtype: Any | None = None
        self.current_link: DependencyChain | None = None
    ...
    def to_dict(self) -> dict:
        ...
        match self.type:
            case 'union' | 'intersection' | 'difference':
                return {
                    self.type: {
                        'left': self.left_chain.to_dict() if self.left_chain else {},
                        'right': self.right_chain.to_dict() if self.right_chain else {},
                    }
                }
            ...
            case 'field':
                ...
                return {
                    self.association.name: {
                        ...
                        'next_association': self.next_link.to_dict()
                        if self.next_link
                        else {},
                    }
                }
            ...
            case 'transitive':
                return {
                    'transitive': self.next_link.to_dict() if self.next_link else {}
                }
...
class LanguageGraph:
    ...
    def process_step_expression(
        self, lang: dict, target_asset, dep_chain, step_expression: dict
    ) -> tuple:
        ...
        if logger.isEnabledFor(logging.DEBUG):
            ...
            logger.debug(
                'Processing Step Expression:\n%s', json.dumps(step_expression, indent=2)
            )
        ...
        match step_expression['type']:
            case 'attackStep':
                ...
                return (target_asset, dep_chain, step_expression['name'])
            ...
            case 'union' | 'intersection' | 'difference':
                ...
                if not lh_target_asset.get_all_common_superassets(rh_target_asset):
                    logger.error(
                        ...
                        lh_target_asset.name,
                        rh_target_asset.name,
                    )
                    return (None, None, None)

                new_dep_chain = DependencyChain(
                    type=step_expression['type'], next_link=None
                )
                new_dep_chain.left_chain = lh_dep_chain
                new_dep_chain.right_chain = rh_dep_chain
                return (lh_target_asset, new_dep_chain, None)
            ...
            case 'variable':
                ...
                if variable_step_expr:
                    return self.process_step_expression(
                        lang, target_asset, dep_chain, variable_step_expr
                    )
            ...
            case 'field':
                ...
                if not target_asset:
                    logger.error('Missing target asset for field "%s"!', fieldname)
                    return (None, None, None)

                new_target_asset = None
                for association in target_asset.associations:
                    if (
                        association.left_field.fieldname == fieldname
                        and target_asset.is_subasset_of(association.right_field.asset)
                    ):
                        new_target_asset = association.left_field.asset

                    if (
                        association.right_field.fieldname == fieldname
                        and target_asset.is_subasset_of(association.left_field.asset)
                    ):
                        new_target_asset = association.right_field.asset

                    if new_target_asset:
                        new_dep_chain = DependencyChain(
                            type='field', next_link=dep_chain
                        )
                        new_dep_chain.fieldname = association.get_opposite_fieldname(
                            fieldname
                        )
                        new_dep_chain.association = association
                        return (new_target_asset, new_dep_chain, None)
                logger.error(
            ...
            case 'transitive':
                ...
                result_target_asset, result_dep_chain, attack_step = (
                    self.process_step_expression(
                        lang, target_asset, dep_chain, step_expression['stepExpression']
                    )
                )
                new_dep_chain = DependencyChain(
                    type='transitive', next_link=result_dep_chain
                )
                return (result_target_asset, new_dep_chain, attack_step)
            ...
            case 'subType':
                ...
                subtype_asset = next(
                    (asset for asset in self.assets if asset.name == subtype_name), None
                )
                ...
                if not subtype_asset.is_subasset_of(result_target_asset):
                    logger.error(
                        ...
                        subtype_name,
                        result_target_asset.name,
                    )
                    return (None, None, None)

                new_dep_chain = DependencyChain(
                    type='subType', next_link=result_dep_chain
                )
                new_dep_chain.subtype = subtype_asset
                return (subtype_asset, new_dep_chain, attack_step)
            ...
            case 'collect':
                ...
                (rh_target_asset, rh_dep_chain, rh_attack_step_name) = (
                    self.process_step_expression(
                        lang, lh_target_asset, lh_dep_chain, step_expression['rhs']
                    )
                )
                return (rh_target_asset, rh_dep_chain, rh_attack_step_name)

            case _:
                logger.error('Unknown attack step type: "%s"', step_expression['type'])
                return (None, None, None)

    def reverse_dep_chain(
        self, dep_chain: DependencyChain | None, reverse_chain: DependencyChain | None
        ...
        match dep_chain.type:
            case 'union' | 'intersection' | 'difference':
                left_reverse_chain = self.reverse_dep_chain(
                    dep_chain.left_chain, reverse_chain
                )
                right_reverse_chain = self.reverse_dep_chain(
                    dep_chain.right_chain, reverse_chain
                )
                new_dep_chain = DependencyChain(type=dep_chain.type, next_link=None)
            ...
            case 'field':
                ...
                new_dep_chain = DependencyChain(type='field', next_link=reverse_chain)
                new_dep_chain.fieldname = opposite_fieldname
                new_dep_chain.association = association
                return self.reverse_dep_chain(dep_chain.next_link, new_dep_chain)
            ...
            case 'subType':
                ...
                new_dep_chain = DependencyChain(
                    type='subType', next_link=result_reverse_chain
                )
    ...
    def _generate_graph(self) -> None:
        ...
        for asset in self._lang_spec['assets']:
            logger.debug(
                'Create asset language graph nodes for asset %s', asset['name']
            )
        ...
        for asset_info in self._lang_spec['assets']:
            asset = next(
                (asset for asset in self.assets if asset.name == asset_info['name']),
                None,
            )
            if asset_info['superAsset']:
                super_asset = next(
                    (
                        asset
                        for asset in self.assets
                        if asset.name == asset_info['superAsset']
                    ),
                    None,
                )
                if not super_asset:
                    msg = 'Failed to find super asset "%s" for asset "%s"!'
                    logger.error(msg, asset_info['superAsset'], asset_info['name'])
        ...
        for asset in self.assets:
            logger.debug(
                'Create association language graph nodes for asset %s', asset.name
            )
            ...
            for association in associations:
                left_asset = next(
                    (
                        asset
                        for asset in self.assets
                        if asset.name == association['leftAsset']
                    ),
                    None,
                )
                if not left_asset:
                    msg = 'Left asset "%s" for association "%s" not found!'
                    logger.error(msg, association['leftAsset'], association['name'])
                    raise LanguageGraphAssociationError(
                ...
                right_asset = next(
                    (
                        asset
                        for asset in self.assets
                        if asset.name == association['rightAsset']
                    ),
                    None,
                )
                if not right_asset:
                    msg = 'Right asset "%s" for association "%s" not found!'
                    logger.error(msg, association['rightAsset'], association['name'])
                    raise LanguageGraphAssociationError(
                ...
                assoc_node = next(
                    (
                        assoc
                        for assoc in self.associations
                        if assoc.name == association['name']
                        and assoc.left_field.asset == left_asset
                        and assoc.right_field.asset == right_asset
                    ),
                    None,
                )
                if assoc_node:
        ...
        for asset in self.assets:
            logger.debug(
                'Create attack steps language graph nodes for asset %s', asset.name
            )
            attack_steps = self._get_attacks_for_asset_type(asset.name)
            for attack_step_name, attack_step_attribs in attack_steps.items():
                logger.debug(
                    'Create attack step language graph nodes for %s', attack_step_name
                )
        ...
        for attack_step in self.attack_steps:
            logger.debug('Determining children for attack step %s', attack_step.name)
            step_expressions = (
                attack_step.attributes['reaches']['stepExpressions']
                if attack_step.attributes['reaches']
                else []
            )
            ...
            for step_expression in step_expressions:
                ...
                (target_asset, dep_chain, attack_step_name) = (
                    self.process_step_expression(
                        self._lang_spec, attack_step.asset, None, step_expression
                    )
                )
                if not target_asset:
                    msg = (
                        'Failed to find target asset to link with for '
                        'step expression:\n%s'
                    )
                ...
                target_attack_step = next(
                    (
                        attack_step
                        for attack_step in target_asset.attack_steps
                        if attack_step.name == attack_step_name
                    ),
                    None,
                )
                ...
                if not target_attack_step:
                    msg = (
                        'Failed to find target attack step %s on %s to '
                ...
                if target_attack_step.name in attack_step.children:
                    attack_step.children[target_attack_step.name].append(
                        (target_attack_step, self.reverse_dep_chain(dep_chain, None))
                    )
                else:
    ...
    def _get_attacks_for_asset_type(self, asset_type: str) -> dict:
        ...
        logger.debug(
            'Get attack steps for %s asset from ' 'language specification.',
            asset['name'],
        )
        if asset['superAsset']:
            logger.debug(
        ...
        for step in asset['attackSteps']:
            if step['name'] not in attack_steps:
                ...
                and 'stepExpressions' in attack_steps[step['name']]['reaches']
            ):
                attack_steps[step['name']]['reaches']['stepExpressions'].extend(
                    step['reaches']['stepExpressions']
                )
            else:
    ...
    def _get_associations_for_asset_type(self, asset_type: str) -> list:
        ...
        if not asset:
            logger.error(
                'Failed to find asset type %s when ' 'looking for associations.',
                asset_type,
            )
            return associations
        ...
        if asset['superAsset']:
            ...
            associations.extend(
                self._get_associations_for_asset_type(asset['superAsset'])
            )
        assoc_iter = (
            assoc
            for assoc in self._lang_spec['associations']
    ...
    def _get_variable_for_asset_type_by_name(
        ...
        if not variable_dict:
            if asset['superAsset']:
                variable_dict = self._get_variable_for_asset_type_by_name(
            ...
            raise LanguageGraphException(msg % (variable_name, asset_type))
    ...
    def get_association_by_fields_and_assets(
        ...
        for assoc in self.associations:
            ...
            if (
                assoc.left_field.fieldname == first_field
                and assoc.right_field.fieldname == second_field
                and first_asset.is_subasset_of(assoc.left_field.asset)
                and second_asset.is_subasset_of(assoc.right_field.asset)
            ):
                return assoc

            if (
                assoc.left_field.fieldname == second_field
                and assoc.right_field.fieldname == first_field
                and second_asset.is_subasset_of(assoc.left_field.asset)
                and first_asset.is_subasset_of(assoc.right_field.asset)
            ):
                return assoc
