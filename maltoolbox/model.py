...
class Model:
    ...
    def get_associated_assets_by_field_name(
        ...
        for association in asset.associations:
            ...
            if opposite_field_name == field_name:
                associated_assets.extend(getattr(association, opposite_field_name))
    ...
    def association_to_dict(self, association: SchemaGeneratedClass) -> dict:
        ...
        association_dict = {
            association.__class__.__name__: {
                str(left_field_name): [int(asset.id) for asset in left_field],
                str(right_field_name): [int(asset.id) for asset in right_field],
            }
        }
    ...
    def _from_dict(
        ...
        for asset_id, asset_object in serialized_object['assets'].items():
            ...
            asset = getattr(model.lang_classes_factory.ns, asset_object['type'])(
