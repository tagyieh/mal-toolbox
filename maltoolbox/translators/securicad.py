...
def load_model_from_scad_archive(
    ...
    instance_model = Model(scad_archive, lang_classes_factory)
    ...
    for child in root.iter('objects'):
        ...
        if child.attrib['metaConcept'] == 'Attacker':
            ...
            instance_model.add_attacker(attacker_at, attacker_id=attacker_obj_id)
            continue
    ...
    for child in root.iter('associations'):
        logger.debug(
            ...
            child.attrib['sourceObject'],
            child.attrib['targetObject'],
            child.attrib['targetProperty'],
            child.attrib['sourceProperty'],
            scad_archive,
        )
        ...
        if attacker_id is not None:
            ...
            if not attacker:
                logger.error(
                    'Failed to find attacker with id %s in model!', attacker_id
                )
                return None
            target_asset = instance_model.get_asset_by_id(target_id)
            if not target_asset:
                logger.error('Failed to find asset with id %s in model!', target_id)
                return None
            attacker.entry_points.append((target_asset, [target_prop.split('.')[0]]))
            continue

        left_asset = instance_model.get_asset_by_id(left_id)
        if not left_asset:
            logger.error('Failed to find asset with id %s in model!', left_id)
            return None
        right_asset = instance_model.get_asset_by_id(right_id)
        if not right_asset:
            logger.error('Failed to find asset with id %s in model!', right_id)
            return None
        ...
        assoc_name = lang_classes_factory.get_association_by_signature(
            lang_graph_assoc.name, left_asset.type, right_asset.type
        )
