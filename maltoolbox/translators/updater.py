...
def load_model_from_older_version(
    filename: str, lang_classes_factory: LanguageClassesFactory, version: str
) -> Model:
    match version:
        case '0.0.39':
            return load_model_from_version_0_0_39(filename, lang_classes_factory)
        case _:
...
def load_model_from_version_0_0_39(
    filename: str, lang_classes_factory: LanguageClassesFactory
) -> Model:
    ...
    def _process_model(model_dict, lang_classes_factory) -> Model:
        ...
        for asset_id, asset_object in model_dict['assets'].items():
            ...
            asset = getattr(model.lang_classes_factory.ns, asset_object['metaconcept'])(
        ...
        for assoc_dict in model_dict.get('associations', []):
            association = getattr(
                model.lang_classes_factory.ns, assoc_dict.pop('metaconcept')
            )()
                ...
                for asset_id in attackers_info[attacker_id]['entry_points']:
                    attacker.entry_points.append(
                        (
                            ...
                            attackers_info[attacker_id]['entry_points'][asset_id][
                                'attack_steps'
                            ],
                        )
                    )
    ...
    def load_from_json(
        ...
        """
        with open(filename, encoding='utf-8') as model_file:
